...
#### ✅ 완료된 작업
- **Phase 1: 테넌시(Tenancy) 기반 구축**: 멀티테넌시 환경의 기반을 마련함.
  - **DB 스키마 확장**: `projects`, `users`, `roles`, `user_project_roles` 테이블을 새로 생성하고, `vms` 테이블에 `project_id`를 추가함. `db_init.py`는 이제 DB 초기화 시 이 모든 스키마와 기본 데이터(admin user, default project)를 생성.
  - **서비스 계층 리팩토링**: `ComputeService`의 핵심 메서드(`create_vm`, `list_vms`, `destroy_vm`)가 `project_id`를 인자로 받도록 수정하고, 모든 DB 쿼리가 프로젝트 단위로 격리되도록 변경.
  - **API 계층 수정**: `app.py`의 API 핸들러들이 `X-Project-ID` 헤더를 통해 `project_id`를 받아 서비스 계층에 전달하도록 수정.
  - **단위 테스트 업데이트**: 서비스 계층의 변경사항에 맞춰 모든 단위 테스트가 `project_id`를 사용하도록 수정하고, 모든 테스트가 통과함을 확인.

#### 🤔 논의된 내용
- **인증/인가 로드맵 구체화**: 오픈스택 모델을 참고하여, 테넌시 기반 구축 이후의 인증(토큰 발급), 인가(역할 기반 접근 제어) 구현을 위한 구체적인 3단계 계획을 수립하고 `next-tasks.md`에 반영함.
